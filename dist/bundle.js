...
const Bus_1 = __webpack_require__(/*! ./Bus */ "./src/components/Bus.tsx");
const Trip_1 = __webpack_require__(/*! ./Trip */ "./src/components/Trip.tsx");
const Header_1 = __webpack_require__(/*! ./Header */ "./src/components/Header.tsx");
class App extends React.Component {
    constructor() {
        super(...arguments);
        this._onTripClick = (tripID) => {
            let busses = this.state.busses;
            let selectedTrip = null;
            let selectedTripBusID = null;
            let busKeys = Object.keys(busses);
            busKeys.map((_, busID) => {
                busses[busID].trips.map((trip, id) => {
        ...
        this._onBusClick = (busID) => {
            ...
            if (this._canMove(this.state.selectedTrip, busID)) {
                ...
                busses[this.state.selectedTripBusID].trips = busses[this.state.selectedTripBusID].trips.filter((t) => t.id != trip.id);
                busses = this._consolidateBusses(busses);
                this.setState({ busses, selectedTrip: null, selectedTripBusID: null });
            }
        };
        this._canMove = (trip, busID) => {
            let trips = this.state.busses[busID].trips;
            for (let i = 0; i < trips.length; i++) {
                if ((trip.startTime >= trips[i].startTime && trip.startTime < trips[i].endTime) ||
        ...
        this._consolidateBusses = (busses) => {
            let newBusses = {};
            let keys = Object.keys(busses).filter((_, key) => busses[key].trips.length != 0);
    ...
    render() {
        let busLanes = Object.keys(this.state.busses).map((_, bus) => {
            let trips = this.state.busses[bus].trips.sort((a, b) => a.startTime - b.startTime);
            let startTime = null;
            let endTime = null;
            if (trips.length > 0) {
            ...
            return (React.createElement(Bus_1.default, { key: bus, id: bus, onBusClick: this._onBusClick, startTime: startTime, endTime: endTime }, this.state.busses[bus].trips.map((t) => React.createElement(Trip_1.default, Object.assign({}, t, { onTripClick: this._onTripClick, key: t.id })))));
        });
        busLanes.unshift(React.createElement(Header_1.default, { schedule: this.props.schedule }));
        return busLanes;
    }
}
exports.default = App;
...
Object.defineProperty(exports, "__esModule", { value: true });
const React = __webpack_require__(/*! React */ "./node_modules/React/index.js");
const interfacetypes_1 = __webpack_require__(/*! ./interfacetypes */ "./src/components/interfacetypes.tsx");
class Bus extends React.Component {
    render() {
        let time = null;
        if (this.props.startTime != null && this.props.endTime != null) {
            let startHour = Math.floor(this.props.startTime / 60);
            let startMin = this.props.startTime % 60;
            let endHour = Math.floor(this.props.endTime / 60);
            let endMin = this.props.endTime % 60;
            time = `${startHour}:${startMin >= 10 ? startMin : "0" + startMin} - ${endHour}:${endMin >= 10 ? endMin : "0" + endMin}`;
        }
        return (React.createElement("div", { className: "bus", onClick: () => this.props.onBusClick(this.props.id) },
            React.createElement("span", { style: { width: interfacetypes_1.TRIP_OFFSET / 2, fontWeight: 'bold' } },
                "Bus ",
                this.props.id),
            React.createElement("span", { style: { width: interfacetypes_1.TRIP_OFFSET / 2 } }, time),
            this.props.children));
    }
}
exports.default = Bus;
...
class Header extends React.Component {
    render() {
        let startHour = null;
        let endHour = null;
        let schedule = this.props.schedule.sort((a, b) => b.endTime - a.endTime);
        if (schedule.length) {
            startHour = 0;
            endHour = Math.ceil(schedule[0].endTime / 60);
        }
        if (startHour != null && endHour != null) {
            let intervals = [...Array(endHour - startHour + 1).keys()];
            return (React.createElement("div", { className: "header" }, intervals.map((i) => React.createElement("span", { className: "interval", style: { left: interfacetypes_1.TRIP_OFFSET + i * 60 } }, `${i >= 10 ? i : "0" + i}:00`))));
        }
...
class Trip extends React.Component {
    render() {
        let width = this.props.endTime - this.props.startTime;
        let cssClass = this.props.selected ? "trip selected" : "trip";
        return (React.createElement("span", { className: cssClass, onClick: (e) => {
                e.stopPropagation();
                this.props.onTripClick(this.props.id);
            }, style: { width, left: this.props.startTime + interfacetypes_1.TRIP_OFFSET } }, this.props.id));
    }
}
...
/***/ "./src/components/interfacetypes.tsx":
/*!*******************************************!*\
  !*** ./src/components/interfacetypes.tsx ***!
  \*******************************************/
/*! no static exports found */
...
const schedule = __webpack_require__(/*! ./input.json */ "./src/input.json");
ReactDOM.render(React.createElement(App_1.default, { schedule: schedule }), document.getElementById("root"));
...
module.exports = [{"id":1,"startTime":30,"endTime":150},{"id":2,"startTime":180,"endTime":300},{"id":3,"startTime":330,"endTime":450},{"id":4,"startTime":40,"endTime":130},{"id":5,"startTime":160,"endTime":250},{"id":6,"startTime":280,"endTime":370},{"id":7,"startTime":400,"endTime":490},{"id":8,"startTime":80,"endTime":240},{"id":9,"startTime":280,"endTime":430}];
